<?php
	...
	$response = array();
	$no_of_unit_list = 0;
	//sales_return_delete.php 
	if(isset($_POST['inv_delete']))
	{
		if(isset($_POST['id']))
		{
			$sql_inv_delete = "SELECT product_id , qty from tbl_sales_return_detail where sales_return_id = '".$_POST['id']."'" ;
			...
			$deleteSql = "CALL deleteSales_return('".$_POST['id']."')";
	...
	//sales_return_product_fetch.php fnc_unit()
	...
	//sales_return_party_fetch.php fnc_party_search() , fnc_state_change()
	else if(isset($_POST['party_search']))
	{
		if(isset($_POST['id']))
		{
		  
		  $party = "select pid.product_id  , un.unit_name  , pm.state , pro.product_name from tbl_sales_invoice pi left join tbl_sales_invoice_detail pid on pid.sales_invoice_id = pi.sales_invoice_id left join tbl_party_master pm on pm.party_id = pi.party_id left join tbl_unit un on un.unit_id = pid.unit_id left join tbl_product_master pro on pro.product_id = pid.product_id WHERE pi.party_id = '".$_POST['id']."' group by pid.product_id ";
	...
	// sales_return_detail_delete.php from remove button
	else if(isset($_POST['edit_delete_detail']))
	{
		if(isset($_POST['id']))
		{
			$sql_inv_fetch = "select product_id , qty from tbl_sales_return_detail where sales_return_detail_id = '".$_POST['id']."'";
			...
			$deleteSql = "delete from tbl_sales_return_detail where sales_return_detail_id = '".$_POST['id']."' ";
			if(mysqli_query($con,$deleteSql))
			{
				$response["Success"] = 1;
			}
	...
	//sales_return_fetch.php
	else if(isset($_POST['edit_fetch']))
	{
		if(isset($_POST['id']))
		{
			$fetchSql = "select * from tbl_sales_return_detail where sales_return_id = '".$_POST['id']."'";
			$result_sql = mysqli_query($con,$fetchSql);
			
			while($row = mysqli_fetch_array($result_sql))
			{
				$sales_return_detail_id = $row["sales_return_detail_id"];
				$product_id = $row["product_id"];
				$unit_id = $row["unit_id"];
				$rate = $row["rate"];
				$qty= $row["qty"];
				...
				$response[] = array ("sales_return_detail_id" => $sales_return_detail_id , "product_id" => $product_id , "unit_id" => $unit_id , "rate" => $rate , "qty" => $qty ,"disc_per" => $disc_per ,"gstslab_id" => $gstslab_id ,"disc_amt" => $disc_amt , "serial_no" => $serial_no , "batch_no" => $batch_no);
			}		
		}
		else
		{
	...
	else if(isset($_POST['edit_unit_fetch']))
	{
		if(isset($_POST['id']))
		{
			$fetchSql = "select * from tbl_sales_return_detail where sales_return_detail_id = '".$_POST['id']."'";
			$result_sql = mysqli_query($con , $fetchSql);
			while($row = mysqli_fetch_array($result_sql))
			{
				$unit_id = $row['unit_id'];
	...
	else if(isset($_POST['max_qty']))
	{
		if(isset($_POST['id']))
		{
			...
			$sql_prd_qty_primary = "select sum(pid.qty) 'qty' , un.unit_id from tbl_sales_invoice pi left join tbl_sales_invoice_detail pid on pid.sales_invoice_id = pi.sales_invoice_id left join tbl_party_master pm on pm.party_id = pi.party_id left join tbl_unit un on  un.unit_id = pid.unit_id left join tbl_company com on com.company_id = pi.company_id where com.is_default = 1 and pi.party_id = '".$_POST['party']."' and pid.product_id = '".$_POST['id']."' GROUP by un.unit_id";
			...
			$sql_qty_return = "select sum(pid.qty) 'qty' , un.unit_id from tbl_sales_return pi left join tbl_sales_return_detail pid on pid.sales_return_id = pi.sales_return_id left join tbl_party_master pm on pm.party_id = pi.party_id left join tbl_unit un on  un.unit_id = pid.unit_id left join tbl_company com on com.company_id = pi.company_id where com.is_default = 1 and pi.party_id = '".$_POST['party']."' and pid.product_id = '".$_POST['id']."' GROUP by un.unit_id";
	...
	else if(isset($_POST['barcode']))
	{
		if(isset($_POST['id']))
		{
			$sql_gst= "select product_name , gstslab_id , sales_rate from tbl_barcode_master where barcode = '".$_POST['id']."' and 
			product_name in 
			(select tbl_sales_invoice_detail.product_id from tbl_sales_invoice_detail 
			left join tbl_sales_invoice on tbl_sales_invoice_detail.sales_invoice_id = tbl_sales_invoice.sales_invoice_id  
			where  tbl_sales_invoice.party_id  = '".$_POST['party']."' )";
			$rs_gst= mysqli_query($con , $sql_gst);
			
			if($rs_gst)
			{
				
				if(mysqli_num_rows($rs_gst) > 0 )
				{
					$row = mysqli_fetch_array($rs_gst);

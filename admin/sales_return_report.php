<?php
$title = "BILL DESK- Sales Return Report";
...
<script>
	function nc_cat(e)
	{
		...
		$.ajax({  
			url:"sales_return_report_ajax.php",
			type:"POST",  
			...
			{  
				...
				if(e.id != 'btn_export')
				{
					...
					{	
						...
						{
							let table_data = "<tr><td style='text-align: center'>"+(i  + 1)   +"</td><td style='text-align: center'>"+data[i].sales_return_date+"</td><td style='text-align: center'>"+data[i].party_name+"</td><td style='text-align: center'>"+data[i].total+"</td><td style='text-align: center'>"+data[i].pay+"</td><td style='text-align: center'>"+data[i].left+"</td><td style='text-align: center'><b><span style='color:"+data[i].color+"' >"+data[i].status+"</span></b></td></tr>";
				...
				else // EXPORT THE TABLE 
				{
					...
					if(e.value == "Excel") // IF EXPORT IN EXCEL
					{
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							var title_report = [['Sales Return Report']];
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							let Heading = [['SR No.','Sales Return Date','Party Name','Total Amount','Pay','Due Amount','Status']];
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							saveAs(new Blob([s2ab(wbout)],{type:"application/octet- stream"}),'Sales_Return_Report'+'_'+curr_date+'_'+curr_month+'_'+curr_year+'.xlsx');
					}
					/*-----------------------------------------------------------------------------------------------------
					...
					{	
						...
						var table_headers = {
							header_0:{
								...
								col_2:{ text: 'Sales Return Date',style: 'table_headers'},
						...
						for (var key in table_data) 
						{
							if (table_data.hasOwnProperty(key))
							{
								...
								row.push( data.sales_return_date.toString()  );
						...
						var docDefinition = {
								...
								[
									...
									{text: 'Sales Return Report', style: 'style_report'},
						...
						// Save PDF 
						pdfMake.createPdf(docDefinition).download('Sales_Return_Report'+'_'+curr_date+'_'+curr_month+'_'+curr_year+'.pdf');
	...
	<div class="col-sm-12">			
		<div class="white-box">
			<div class="panel panel-info">
				<div class="panel-heading">Sales Return Report</div>
			</div>	
			<div class="row">
				<div class="col-md-12">

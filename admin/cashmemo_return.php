...
foreach ($row_product_setting as $key => $value)
{
   $setting->set_name( $value , $key );
	...
	if(isset($_GET['id']))
	{
		...
		$cashmemo_return_id=base64_decode($_GET['id']);
		$sql_cashmemo_return_select="SELECT * FROM tbl_cashmemo_return WHERE cashmemo_return_id ='".$cashmemo_return_id."'";
		$rs_cashmemo_return_select=mysqli_query($con,$sql_cashmemo_return_select);
		$row_cashmemo_return_select=mysqli_fetch_array($rs_cashmemo_return_select);
		
		//fetch data for invoice detail
		$sql_cashmemo_return_detail_select = "select * from tbl_cashmemo_return_detail where cashmemo_return_id = '".$cashmemo_return_id."'";
		$rs_cashmemo_return_detail_select = mysqli_query($con , $sql_cashmemo_return_detail_select);
		$detail_row_num = mysqli_num_rows($rs_cashmemo_return_detail_select);
	...
	//invoice_no sql
	$sql_invoice = "select max(pr.invoice_no) 'invoice_no' from tbl_cashmemo_return pr join tbl_financial_master fy on fy.financial_id = pr.financial_id where fy.is_default = 1";
	...
	{
		global $row_cashmemo_return_select;
		
		if(isset($_GET['id']))
		{
			if($row_cashmemo_return_select['state_of_supply'] == $name)
			{echo "selected";}
		}
	}
...
//``````````````````````````````````````````````````````INSERT```````````````````````````````````````````````````````````````````
	if(isset($_POST['btn_save']) || isset($_POST['btn_save_n_print']))
	{
		...
		$sql_inv_id = "select max(cashmemo_return_id ) 'inv' from tbl_cashmemo_return";
		//echo $sql_inv_id;
		...
		//payment out 
		$objname = "cashmemo_return";
		
		$image="";
		$sql_payment_out = "insert into tbl_payment_out(company_id,financial_id,party_id,receipt_no,payment_type_id,cheque_ref_no,date,description,image,paid,obj_name,obj_id) values('".$row_company_id['company_id']."','".$row_financial_id['financial_id']."','".$_POST['txt_customer_name']."','".$_POST['txt_invoice_no']."','".$_POST['cmb_payment_type']."','".$_POST['txt_cheque_ref']."','".$_POST['txt_invoice_date']."','".$_POST['txt_narration']."','".$image."','".$_POST['txt_pay']."','".$objname."','".$inv_id."')";
		  ...
		  for($i=0; $i<$number; $i++)  
			{  
			   if(trim($_POST["txt_rate"][$i] != ''))
				{  
					...
					$sql_stock_cashmemo = "update tbl_product_master set closing_stock = (select closing_stock from tbl_product_master where product_id ='". $_POST["txt_product"][$i]."' ) - '".$qty_for_stock."' where product_id = '". $_POST["txt_product"][$i]."' ";
			...
			else
			{
				if(isset($_POST['btn_save_n_print']))
					echo "<script>window.location.href='invoice.php?cashmemo_return_id=$inv_id';</script>";
				else
					echo "<script>window.location.href='cashmemo_return.php';</script>";
			}
		}
	}
//``````````````````````````````````````````````````````UPDATE```````````````````````````````````````````````````````````````````
	if(isset($_POST['btn_edit']) || isset($_POST['btn_update_n_print']))
	{
        ...
        $objname = "cashmemo_return";
		$img3 = '';
		$sql_cashmemo_return_update = "UPDATE tbl_cashmemo_return SET customer_name = '".$_POST['txt_customer_name']."',customer_mobile = '".$_POST['txt_customer_mobile']."',state_of_supply ='".$_POST['cmb_state_of_supply']."' , cashmemo_return_date = '".$_POST['txt_invoice_date']."' , payment_type_id = '".$_POST['cmb_payment_type']."', narration= '".$_POST['txt_narration']."', sub_total = '".$_POST['txt_sub_total']."', shipping_packing_amount= '".$_POST['txt_shipping_amt']."' , is_round_off = '".$chk_round_off."', round_off = '".$_POST['txt_round_off']."' , total = '".$_POST['txt_total_inv']."', pay = '".$_POST['txt_pay']."' , new_invoice_no ='".$new_inv_no."' ,is_gst_bill = '".$row_product_setting['is_gst_bill']."' WHERE cashmemo_return_id = '".$_POST['cashmemo_return_id']."'";
		$rs_cashmemo_return_update = mysqli_query($con , $sql_cashmemo_return_update);
		echo $sql_cashmemo_return_update;
		
		$sql_payment_out_update = "update tbl_payment_in set company_id ='".$row_company_id['company_id']."',financial_id='".$row_financial_id['financial_id']."',party_id='".$_POST['txt_customer_name']."',payment_type_id='".$_POST['cmb_payment_type']."',cheque_ref_no='".$cheque."',date='".$_POST['txt_invoice_date']."',description='".$_POST['txt_narration']."',image='".$img3."',paid ='".$_POST['txt_pay']."',obj_name='".$objname."',obj_id = '".$_POST['cashmemo_return_id']."' where party_id ='".$_POST['txt_customer_name']."' and obj_name = '".$objname."' and obj_id = '".$_POST['cashmemo_return_id']."'";
		...
		$sql_company_ledger="update tbl_company_ledger set company_id = '".$row_company_id['company_id']."',related_id = '".$_POST['cashmemo_return_id']."',related_obj_name='".$objname."',party_id = '".$_POST['txt_customer_name']."',date = '".$_POST['txt_invoice_date']."' ,details = '".$_POST['txt_narration']."' , credit = '".$credit."' , debit = '".$_POST['txt_pay']."' ,financial_id = '".$row_financial_id['financial_id']."' where related_id = '".$_POST['cashmemo_return_id']."' and related_obj_name = '".$objname."' ";
		  $rs_company_ledger = mysqli_query($con,$sql_company_ledger);
		...
		{  
		  for($i=0; $i<$number; $i++)  
		  {
		   ...
		   if($_POST["cashmemo_return_detail_id"][$i] == '')		   
			{
				//serial_no is available or not. checking
				$sr = '';
				if($_POST['txt_serial_no'][$i] != '')
			...
			else
			{
				if(isset($_POST['btn_update_n_print']))
					echo "<script>window.location.href='invoice.php?cashmemo_return_id=$cashmemo_return_id';</script>";
				else
					echo "<script>window.location.href='cashmemo_return.php';</script>";
			}
		}
...
var swit = 0;
	...
	function fnc_unit(e)
	{
		...
		let par =  document.getElementById("txt_customer_name").value;
		let prd_detail_id = document.getElementsByName("cashmemo_return_detail_id[]");
	...
	{
		let e = document.getElementById("txt_customer_name");
...
function fnc_prd_fill()
{
  ...
  function fnc_qty_maintain()
  {  
  //console.log("hello motot");
	var prd_len = document.getElementsByName('txt_product[]');
	  for(i=0;i<prd_len.length;i++)
	  {
		var a = document.getElementsByName('txt_quantity[]')[i].valueAsNumber;
		var b = document.getElementsByName('txt_quantity_limit[]')[i].value;
		//console.log(a);
		//console.log(b);
		if(a > b )
		{
			document.getElementsByName('txt_quantity[]')[i].value = document.getElementsByName('txt_quantity_limit[]')[i].value;
	...
	function fnc_chk_batch(e)
	{
		if(e.checked == 1)
		{
			...
			else
			{
				$('#btn_add').click();
				let prd = document.getElementsByName('txt_product[]');
				prd[prd.length - 1].value = prd[batch_key].value;
...
<div class="row">
	<div class="col-md-12">
		<div class="panel panel-info">
			<div class="panel-heading">CASHMEMO RETRUN</div>
					...
					<form action="" method="post" name="add_name" id = "add_name" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="col-md-3">
									<div class="form-group" id = "switch_party">
										...
										<input type="text"id="txt_customer_name" name="txt_customer_name"  class="form-control" placeholder="Enter Customer Name" onchange="fnc_party();" value="<?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['customer_name']; }?>">							
									</div>
								</div>
								<div class="col-md-2">
									<div class="form-group" id = "switch_party">
										<label class="control-label">Customer Mobile</label>
										<input type="text"id="txt_customer_mobile" name="txt_customer_mobile"  class="form-control" placeholder="Enter Customer Mobile" value="<?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['customer_mobile']; }?>">							
									</div>
								</div>
								<!--/span-->
								<div class="col-md-2">
									<div class="form-group">
										...
										<input type="text" id="txt_invoice_no" name="txt_invoice_no" class="form-control" placeholder="Enter Invoice No." readonly = "true" value="<?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['invoice_no']; }else { echo $invoice_no; } ?>">										
									</div>
								</div>
								<!--/span-->
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="hidden" name="is_out_off_state" id="is_out_off_state" value="<?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['out_of_state']; }?>"/>
									</div>
								</div>
								<!--/span-->
								<div class="col-md-2">
									<div class="form-group">
										<label class="control-label">Cashmemo Return Date</label>
							...
							<div class = "row">
								...
								<?php	if(!isset($_GET['id']))
									...
									<tr id='row0'>
										...
										<td>
											<div class="form-group" >
												<select class="form-control first_ddl"  id="txt_product" name="txt_product[]" onchange="fnc_unit(this)" ></select>
											</div>
										</td>
										...
										<td>
											<div class="form-group">
												<input type="number" id="txt_quantity" name="txt_quantity[]" step="0.001" value = "0" oninput = "fnc_qty_serial_count(this)" onchange = "tot(); fnc_qty_maintain();" onkeyup="fnc_qty_maintain();"  class="form-control" placeholder="Enter QUANTITY" >										
										...
										<td>
											<div class="form-group">
												<input type="number" id="txt_rate" name="txt_rate[]" step="0.01" onchange = "tot()"  class="form-control" placeholder="Enter rate" >										
											</div>
										</td>
										<td>
											<div class="form-group">
												<input type="number" id="txt_discount" name="txt_discount[]" step="0.01" onchange = "tot()"  class="form-control" placeholder="Enter discount" >										
											</div>
										</td>
										<td>
											<div class="form-group">
										...
										<input type="hidden" name="cashmemo_return_detail_id[]" id="cashmemo_return_detail_id" value = ''/> 
						...
						<div class="row">
								...
								<div class="form-group">
									...
									<select id="cmb_payment_type" name="cmb_payment_type" class="form-control" onchange="fnc_payment_type_text();">
										<?php 
											...
											{ 
										?>
											<option value="<?php echo $row_payment['payment_type_id']; ?>" <?php if(isset($_GET['id'])){if($row_cashmemo_return_select['payment_type_id'] == $row_payment['payment_type_id']){echo "selected";}}?> ><?php echo $row_payment['payment_type'];?></option>
										<?php
											}
										?>
									</select>										
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<input type="number" id="txt_shipping_amt" name="txt_shipping_amt" class="form-control" value = "<?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['shipping_packing_amount']; }else {echo "0.00";}?>" onchange = "fnc_calculate_total()" placeholder="Enter Shipping/Packing Amount">																			
								</div>
							</div>
							<!--/span-->
						</div>	
							<div class="row">	
								...
								<div class="form-group">
									...
									<textarea id="txt_narration" name="txt_narration" class="form-control" placeholder="Enter Narration"><?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['narration']; }?></textarea>
								</div>
							</div>
								<!--/span-->
								<div class="col-md-4">
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										<input type="checkbox" id="chk_is_round_off" name="chk_is_round_off" onchange = "fnc_calculate_total()" <?php if(isset($_GET['id'])){  if($row_cashmemo_return_select['is_round_off'] == 1){ echo "checked";}}?>>	
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input type="number" id="txt_pay" name="txt_pay" class="form-control"  placeholder="Enter Pay Amount" value="<?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['pay']; }?>" >	
									</div>
								</div>
								<!--/span-->
							</div>
						...
						<div class = "row">
							<div  class = "col-md-8"> </div>
								<input type="hidden" name="cashmemo_return_id" id="cashmemo_return_id" value = "<?php if(isset($_GET['id'])){ echo $row_cashmemo_return_select['cashmemo_return_id']; }?>"/>
								<?php	if(isset($_GET['id']))
										{ ?>
...
$(document).ready(function(){
	...
	$('#txt_customer_name,#txt_invoice_date, #txt_customer_mobile').on('keyup , change', function () {
		$(".error_company").remove();
	});
	  
	$(document).on('click', '#btn_add', function() {
		  
		$('#dynamic_field').append('<tr id="row"><?php if($setting->get_name('is_show_barcode') == 1) { ?><td><div class="form-group"><input type="number"  oninput="barcode_data(this)" name="barcode_id[]" id="barcode_id" class = "form-control" value = ""/></div></td><?php } ?><td><div class="form-group"><select class="form-control" id="txt_product" name="txt_product[]" onchange="fnc_unit(this)" placeholder="select Product"></select></div></td><?php if($setting->get_name('is_show_serial') == 1) { ?><td><div class="form-group"><a id="link_serial" onclick="fnc_serial_find(this);" name="link_serial[]" class="form-control" data-toggle="modal" data-target="#serial" style="pointer-events: none;">Serial</a></div></td><?php } ?><?php if($setting->get_name('is_show_batch') == 1) { ?><td><div class="form-group"><a id="link_batch" name="link_batch[]" onclick="fnc_batch_find(this);" class="form-control" data-toggle="modal" data-target="#batch" style="pointer-events: none;">Batch</a></div></td><?php } ?><td><div class="form-group"><select class="form-control" id="product_unit" name="product_unit[]" onchange="fnc_limit_change(this);fnc_rate_update(this);"></select></div></td><td><div class="form-group"><input type="number" id="txt_quantity" name="txt_quantity[]" step="0.001" value="0" onchange="tot(); " oninput="fnc_qty_serial_count(this); fnc_qty_maintain();" class="form-control" placeholder="Enter QUANTITY"><input type="hidden" id="txt_quantity_limit" name="txt_quantity_limit[]"><input type="hidden" id="share_rate" name="share_rate[]"></div></td><td><div class="form-group"><input type="number" id="txt_rate" name="txt_rate[]" step="0.01" onchange="tot()" class="form-control" placeholder="Enter rate"></div></td><td><div class="form-group"><input type="number" id="txt_discount" name="txt_discount[]" step="0.01" onchange="tot()" class="form-control"  value=0></div></td><td><div class="form-group"><select id="txt_gst" name="txt_gst[]" class="form-control" onchange="fnc_gst_cal(this);"><option value="">--select--</option><?php $sql_gst="SELECT gst.* FROM tbl_gstslab_master gst LEFT JOIN tbl_company com ON com.company_id  = gst.company_id  WHERE com.is_default = 1 "; $rs_gst = mysqli_query($con,$sql_gst); while($row_gst=mysqli_fetch_array($rs_gst)){ ?><option value="<?php echo $row_gst['gstslab_id']; ?>"><?php echo $row_gst['gstslab_name']; ?></option><?php } ?></select></div></td><td><div class="form-group"><input type="number" id="txt_total" name="txt_total[]" step="0.01" readonly="true" class="form-control" placeholder="Enter TOTAL"></div></td><td><input type="hidden" name="txt_cgst[]" id="txt_cgst" value = "0"/><input type="hidden" name="txt_unit[]" id="txt_unit" /><input type="hidden" name="txt_discount_amt[]" id="txt_discount_amt" /><input type="hidden" name="txt_serial_no[]" id="txt_serial_no" value=""> <input type="hidden" name="txt_batch_no[]" id="txt_batch_no" value=""><input type="hidden" name="cashmemo_return_detail_id[]" id="cashmemo_return_detail_id" /><input type="hidden" name="txt_batch_stonks[]" id="txt_batch_stonks" value="0"> <button type="button" name="btn_remove[]" id="btn_remove" class="btn btn-danger btn_remove">X</button></td></tr>');
		fnc_prd_fill();
		let fcs = document.getElementsByName('barcode_id[]');
		fcs[fcs.length - 1].focus();

	});  
	  
	//remove button code
	$(document).on('click', '.btn_remove', function(){  
	    let detail_id = $(this).siblings("#cashmemo_return_detail_id").val();
		if(detail_id != '')
		{
	...
	success: function(data) 
	{
		for(i=0;i<no_of_detail_row;i++)
		{
			 var detail_id = $(this).siblings("#cashmemo_return_detail_id").val();
			//console.log(data[i]);
			...
			document.getElementsByName("cashmemo_return_detail_id[]")[i].value = data[i].cashmemo_return_detail_id;
			...
			fnc_unit_refresh(document.getElementsByName("cashmemo_return_detail_id[]")[i]);

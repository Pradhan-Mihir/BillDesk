<?php
$title = "BILL DESK- Purchase Report";
			...
			{  
				...
				{	
					...
					{	
						...
						{
							let table_data = "<tr><td style='text-align: center'>"+( i  + 1)   +"</td><td style='text-align: center'>"+data[i].purchase_invoice_date+"</td><td style='text-align: center'>"+data[i].party_name+"</td><td style='text-align: center'>"+data[i].total+"</td><td style='text-align: center'>"+data[i].pay+"</td><td style='text-align: center'>"+data[i].left+"</td><td style='text-align: center'><b><span style='color:"+data[i].color+"' >"+data[i].status+"</span></b></td></tr>";
				...
				else // EXPORT THE TABLE 
				{
					...
					if(e.value == "Excel") // IF EXPORT IN EXCEL
					{
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							var title_report = [['Purchase Report']];
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							let Heading = [['SR No.','Purchase Date','Party Name','Total Amount','Pay','Due Amount','Status']];
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							saveAs(new Blob([s2ab(wbout)],{type:"application/octet- stream"}),'Purchase_Report'+'_'+curr_date+'_'+curr_month+'_'+curr_year+'.xlsx');
					}
					/*-----------------------------------------------------------------------------------------------------
					...
					{	
						...
						var table_headers = {
							header_0:{
								...
								col_2:{ text: 'Purchase Date',style: 'table_headers'},
						...
						for (var key in table_data) 
						{
							if (table_data.hasOwnProperty(key))
							{
								...
								row.push( data.purchase_invoice_date.toString()  );
						...
						var docDefinition = {
								...
								[
									...
									{text: 'Purchase Report', style: 'style_report'},
						...
						// Save PDF 
						pdfMake.createPdf(docDefinition).download('Purchase_Report'+'_'+curr_date+'_'+curr_month+'_'+curr_year+'.pdf');

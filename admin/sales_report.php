            ...
            success:function(data)
            {
                if(e.id != 'btn_export')
				{
					if(data[0].total_records != 0)
					{
						...
						{
							let table_data = "<tr><td style='text-align: center'>"+( i  + 1)   +"</td><td style='text-align: center'>"+data[i].sales_invoice_date+"</td><td style='text-align: center'>"+data[i].party_name+"</td><td style='text-align: center'>"+data[i].total+"</td><td style='text-align: center'>"+data[i].pay+"</td><td style='text-align: center'>"+data[i].left+"</td><td style='text-align: center'><b><span style='color:"+data[i].color+"' >"+data[i].status+"</span></b></td></tr>";
				...
				else // EXPORT THE TABLE 
				{
					...
					if(e.value == "Excel") // IF EXPORT IN EXCEL
					{
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							let Heading = [['SR No.','Sales Date','Party Name','Total Amount','Pay','Due Amount','Status']];
						...
						/*-----------------------------------------------------------------------------------------------*/
							...
							saveAs(new Blob([s2ab(wbout)],{type:"application/octet- stream"}),'Sales_Report'+'_'+curr_date+'_'+curr_month+'_'+curr_year+'.xlsx');
					}
					/*-----------------------------------------------------------------------------------------------------
					...
					{	
						...
						var table_headers = {
							header_0:{
								...
								col_2:{ text: 'Sales Date',style: 'table_headers'},
						...
						for (var key in table_data)
						{
							if (table_data.hasOwnProperty(key))
							{
								...
								row.push( data.sales_invoice_date.toString()  );
						...
						var docDefinition = {
								...
								[
									...
									{text: 'Sales Report', style: 'style_report'},
						...
						// Save PDF 
						pdfMake.createPdf(docDefinition).download('Sales_Report'+'_'+curr_date+'_'+curr_month+'_'+curr_year+'.pdf');
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <select id="cmb_party" name="cmb_party" class="form-control select2" onchange="nc_cat(this)">
                                            ...
                                            <?php
                                            $sql_party="select * from tbl_party_master where party_type = 1";
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <input type="date" id="start_date" name="start_date" class="form-control" onchange = "nc_cat(thiss)">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            url:"sales_report_ajax.php",
            type:"POST",
            data:{ 'id' : 1,'party': 0 ,'start_date' : '' , 'end_date': '', 'status' : 0 , 'pagging' : 1},
            dataType :'json',
            success:function(data)
            {

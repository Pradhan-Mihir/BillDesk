<?php
$title = "BILL DESK-Sales Invoice";
	include_once('header.php');
	
	$detail_row_num = 0;
	$sales_id = 0;
	$quotation_id = 0;
	$detail_row_num_quotation = 0;
...
// $setting->get_name('sales');
//echo $setting->get_name('is_show_serial');
//fetch data for update
	if(isset($_GET['id']))
	{
		//fetch data for invoice
		$sales_id=base64_decode($_GET['id']);
		$sql_sales_select="SELECT * FROM tbl_sales_invoice WHERE sales_invoice_id ='".$sales_id."'";
		$rs_sales_select=mysqli_query($con,$sql_sales_select);
		$row_sales_select=mysqli_fetch_array($rs_sales_select);
		
		//fetch data for invoice detail
		$sql_sales_detail_select = "select * from tbl_sales_invoice_detail where sales_invoice_id = '".$sales_id."'";
		$rs_sales_detail_select = mysqli_query($con , $sql_sales_detail_select);
		$detail_row_num = mysqli_num_rows($rs_sales_detail_select);
		
		
	}
	if(isset($_GET['quotation_to_sales']))
	{
		//fetch data for invoice
		$quotation_id =base64_decode($_GET['quotation_to_sales']);
		$sql_quotation="SELECT * FROM tbl_quotation WHERE quotation_id ='".$quotation_id."'";
		$rs_quotation=mysqli_query($con,$sql_quotation);
		$row_quotation=mysqli_fetch_array($rs_quotation);
		
		//fetch data for invoice detail
		$sql_quotation_detail = "select * from tbl_quotation_detail where quotation_id = '".$quotation_id."'";
		$rs_quotation_detail = mysqli_query($con , $sql_quotation_detail);
		$detail_row_num_quotation = mysqli_num_rows($rs_quotation_detail);
	...
	//invoice_no sql
	$sql_invoice = "select max(pr.invoice_no) 'invoice_no' from tbl_sales_invoice pr join tbl_financial_master fy on fy.financial_id = pr.financial_id where fy.is_default = 1";
	...
	{
		global $row_sales_select;
		
		if(isset($_GET['id']))
		{
			if($row_sales_select['state_of_supply'] == $name)
			{echo "selected";}
		}
	}
	
	$new_inv_no = $setting->get_name('sales') . $row_financial_id['financial_year'];
	//```````````````````````````````````````````````````INSERT````````````````````````````````````````````````//
	if(isset($_POST['btn_save']) || isset($_POST['btn_save_n_print']))
	{
		if(isset($_POST['chk_is_round_off']))
            ...
            $chk_round_off=0;
		//INSERT CODE
		$sql_sales_invoice_iu = "insert into tbl_sales_invoice (company_id,party_id,invoice_no,out_of_state,state_of_supply,sales_invoice_date,payment_type_id,narration,sub_total,shipping_packing_amount,is_round_off,round_off,total,pay,financial_id,new_invoice_no,is_gst_bill) values('".$row_company_id['company_id']."' ,'".$_POST['cmb_party']."' ,'".$_POST['txt_invoice_no']."','".$_POST['is_out_off_state']."' ,'".$_POST['cmb_state_of_supply']."' ,'".$_POST['txt_invoice_date']."' ,'".$_POST['cmb_payment_type']."' ,'".$_POST['txt_narration']."' ,'".$_POST['txt_sub_total']."' ,'".$_POST['txt_shipping_amt']."' ,'".$chk_round_off."' ,'".$_POST['txt_round_off']."' ,'".$_POST['txt_total_inv']."' ,'".$_POST['txt_pay']."' ,'".$row_financial_id['financial_id']."', '".$new_inv_no."','".$row_product_setting['is_gst_bill']."') ";
		
		$rs_sales_invoice_iu = mysqli_query($con , $sql_sales_invoice_iu);
		
		if(! $rs_sales_invoice_iu )
		...
		$sql_payment_in = "insert into tbl_payment_in(company_id,financial_id,party_id,receipt_no,payment_type_id,cheque_ref_no,date,description,image,received,obj_name,obj_id) values('".$row_company_id['company_id']."','".$row_financial_id['financial_id']."','".$_POST['cmb_party']."','".$_POST['txt_invoice_no']."','".$_POST['cmb_payment_type']."','".$txt_che."','".$_POST['txt_invoice_date']."','".$_POST['txt_narration']."','".$image."','".$_POST['txt_pay']."','".$objname."','".$inv_id."')";
		$rs_payment_in = mysqli_query($con,$sql_payment_in);
		
		//company_ledger
		$debit=0.00;
		
		$sql_company_ledger="insert into tbl_company_ledger(company_id , related_id , related_obj_name,party_id ,date, details, credit, debit, financial_id, new_invoice_no) values('".$row_company_id['company_id']."' , '".$inv_id."' ,'".$objname."','".$_POST['cmb_party']."' , '".$_POST['txt_invoice_date']."' , '".$_POST['txt_narration']."' , '".$_POST['txt_pay']."','".$debit."','".$row_financial_id['financial_id']."','".$new_inv_no."')";
		...
		{  
			...
			{		
				...
				{   echo "if for ka ";
					...
					{
						$sql_batch = "UPDATE tbl_batch_tracking SET qty = qty - ".$_POST['txt_quantity'][$i]." WHERE batch_tracking_id  = '".$_POST['txt_batch_no']."' ";
					...
					$sql_stock_sales = "update tbl_product_master set closing_stock = (select closing_stock from tbl_product_master where product_id ='".mysqli_real_escape_string($con, $_POST["txt_product"][$i])."' ) - '".mysqli_real_escape_string($con, $_POST["txt_quantity"][$i])."' where product_id = '".mysqli_real_escape_string($con, $_POST["txt_product"][$i])."' ";
					$rs_stock_sales = mysqli_query($con,$sql_stock_sales);
			...
			else
			{
				if(isset($_POST['btn_save_n_print']))
					echo "<script>window.location.href='invoice.php?sales_id=$inv_id';</script>";
				else
	...
	if(isset($_POST['btn_edit']) || isset($_POST['btn_update_n_print']))
	{
		...
		$objname = "sales";	
		$img3 = "";		
		$sql_sales_invoice_iu = "UPDATE tbl_sales_invoice SET party_id = '".mysqli_real_escape_string($con,$_POST['cmb_party'])."', state_of_supply ='".$_POST['cmb_state_of_supply']."',sales_invoice_date = '".$_POST['txt_invoice_date']."' , payment_type_id = '".$_POST['cmb_payment_type']."', narration= '".$_POST['txt_narration']."', sub_total = '".$_POST['txt_sub_total']."', shipping_packing_amount= '".$_POST['txt_shipping_amt']."' , is_round_off = '".$_POST['chk_is_round_off']."', round_off = '".$_POST['txt_round_off']."' , total = '".$_POST['txt_total_inv']."', pay = '".$_POST['txt_pay']."' , new_invoice_no ='".$new_inv_no."',is_gst_bill = '".$row_product_setting['is_gst_bill']."' WHERE sales_invoice_id = '".$sales_id."'";
		
		$rs_sales_invoice_update = mysqli_query($con , $sql_sales_invoice_iu);
		
		$sql_payment_in_update = "update tbl_payment_in set company_id ='".$row_company_id['company_id']."',financial_id='".$row_financial_id['financial_id']."',party_id='".$_POST['cmb_party']."',payment_type_id='".$_POST['cmb_payment_type']."',date='".$_POST['txt_invoice_date']."',description='".$_POST['txt_narration']."',image='".$img3."',received='".$_POST['txt_pay']."',obj_name='".$objname."',obj_id = '".$sales_id."' where party_id ='".$_POST['cmb_party']."' and obj_name = '".$objname."' and obj_id = '".$sales_id."' ";
		...
		$sql_company_ledger="update tbl_company_ledger set company_id = '".$row_company_id['company_id']."',related_id = '".$_POST['sales_invoice_id']."',related_obj_name='".$objname."',party_id = '".$_POST['cmb_party']."',date = '".$_POST['txt_invoice_date']."' ,details = '".$_POST['txt_narration']."' , credit = '".$_POST['txt_pay']."' , debit = '".$debit."',financial_id = '".$row_financial_id['financial_id']."' where related_id = '".$_POST['sales_invoice_id']."' and related_obj_name = '".$objname."' ";
		$rs_company_ledger = mysqli_query($con,$sql_company_ledger);
		...
		$sql_party_ledger = "update tbl_party_ledger set company_id='".$row_company_id['company_id']."',party_type='".$party_typ."',party_id='".$_POST['cmb_party']."',invoice_type='".$inv_typ."',invoice_no='".$_POST['sales_invoice_id']."',detail='".$_POST['txt_narration']."',credit='".$_POST['txt_pay']."',debit='".$_POST['txt_total_inv']."',date='".$_POST['txt_invoice_date']."',financial_id='".$row_financial_id['financial_id']."',new_invoice_no='".$new_inv_no."' where invoice_no='".$_POST['sales_invoice_id']."' and invoice_type = 'sales'";
		$rs_party_ledger = mysqli_query($con , $sql_party_ledger);
		...
		$sql_fetch_bal_old = "select pay from tbl_sales_invoice where sales_invoice_id = '".$_POST['sales_invoice_id']."' ";
		   ...
		   {  
			...
			else
			{
				if(isset($_POST['btn_update_n_print']))
					echo "<script>window.location.href='invoice.php?sales_id=$sales_id';</script>";
				else
					echo "<script>window.location.href='sales.php';</script>";
			}
		}  
...
var no_of_detail_row_quotation = "<?php echo $detail_row_num_quotation; ?>";
console.log(no_of_detail_row_quotation);
var serial_key;
var batch_key;
var swit = 0 ;
	...
	function fnc_unit(e)
	{
		...
		let prd_detail_id = document.getElementsByName("sales_invoice_detail_id[]");
			...
			{  
				...
				for(i=0;i<prd_len;i++)
				{
					...
					if(prd[i] == e)
					{
						...
						//setting batch is visible or not
                        if(is_batc == 1)
						{
	...
	function fnc_batch_find(e)
	{
		...
		for(i=0;i<bat.length;i++)
		{
			if(e == bat[i])
			{
				...
				if(document.getElementsByName('sales_invoice_detail_id[]')[batch_key].value != '')
				{
					$.ajax({
					url:"sales_ajax.php",
					type:"POST", 
					data:{ 'id' : prd[i].value , 'batch_fill' : 1  , 'edit_time' : document.getElementsByName('sales_invoice_detail_id[]')[batch_key].value},
					success:function(data)  
					{  
	...
	function fnc_serial_find(e)
	{
		...
		for(i=0;i<sr.length;i++)
		{
			if(e == sr[i])
			{
				...
				if(document.getElementsByName('sales_invoice_detail_id[]')[serial_key].value != '')
				{
					$.ajax({  
					url:"sales_ajax.php",
					type:"POST", 
					data:{ 'id' : prd[i].value , 'serial_fill' : 1 , 'edit_time' : document.getElementsByName('sales_invoice_detail_id[]')[serial_key].value },
					success:function(data)  
					{  
						const obj = JSON.parse(data);
						$('#serial_body').html('');
						...
						for(i=0 ; i < obj.length ; i++)
						{
								$('#serial_body').append("<tr id='row_serial'><td align='center'><input type='checkbox' checked disabled id='serial_no_id' name='serial_no_id[]' onchange = 'fnc_chk_serial(this);' value = '"+obj[i].serial_no_id+"'></td><td align='center'><label class = 'control-label' id='txt_serial' name='txt_serial[]' value = '"+obj[i].serial_no+"' >"+obj[i].serial_no+"</label></td></tr>");
...
function fnc_validation()
{
    ...
    let cmb_party = document.getElementById('cmb_party');
					...
					<form  method="post" name="add_name" id = "add_name" enctype="multipart/form-data">
							...
							<div class="row">
									...
									<div class="form-group" id = "switch_party">
										...
										<select id="cmb_party" name="cmb_party"  class="form-control" onchange="fnc_party();javascript:fnc_add_party_select(this);">
											...
											<?php 
												...
												{ 
											?>
												<option value="<?php echo $row_party['party_id']; ?>" <?php if(isset($_GET['id'])){if($row_sales_select['party_id'] == $row_party['party_id']){echo "selected";}}?><?php if(isset($_GET['quotation_to_sales'])){if($row_quotation['party_id'] == $row_party['party_id']){echo "selected";}}?> ><?php echo $row_party['party_name'];?></option>
											<?php
												}
											?>
										</select>								
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="text" id="txt_invoice_no" name="txt_invoice_no" class="form-control" placeholder="Enter Invoice No." readonly = "true"value="<?php if(isset($_GET['id'])){ echo $row_sales_select['invoice_no']; }else { echo $invoice_no; } ?>">										
									</div>
								</div>
								<!--/span-->
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="hidden" name="is_out_off_state" id="is_out_off_state" value="<?php if(isset($_GET['id'])){ echo $row_sales_select['out_of_state']; }?>"/>
									</div>
								</div>
								<!--/span-->
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="date" id="txt_invoice_date" name="txt_invoice_date" class="form-control" onclick="fnc_"  value="<?php if(isset($_GET['id'])){ echo $row_sales_select['sales_invoice_date']; }else { print(date("Y-m-d")); } ?>">		
									</div>
								</div>
								<!--/span-->
								<!--/span-->
							...
							<div class = "row">
								...
								<?php	if(!isset($_GET['id']))
									...
									<tr id='row0'>
										...
										<td>
											<div class="form-group">
												<input type="number" id="txt_rate" name="txt_rate[]" onchange = "tot();fnc_style_remove(this);"  class="form-control" placeholder="Enter rate" >				
											</div>
										</td>
										<td>
											<div class="form-group">
												<input type="number" id="txt_discount" name="txt_discount[]" onchange = "tot();fnc_style_remove(this);"  class="form-control" placeholder="Enter discount" value = 0>										
											</div>
										</td>
										<td>
											<div class="form-group">
										...
										<td>
											<div class="form-group">
												<input type="number" id="txt_total" name="txt_total[]" readonly = "true" class="form-control" placeholder="Enter TOTAL" >										
											</div>
										</td>
										<td>
										...
										<input type="hidden" name="sales_invoice_detail_id[]" id="sales_invoice_detail_id" value = ''/> 
						...
						<div class="row">
							...
							<div class="row">
									...
									<div class="form-group">
										...
										<select id="cmb_payment_type" name="cmb_payment_type" class="form-control" onchange="fnc_payment_type_text();">
											<?php 
												...
												{ 
											?>
												<option value="<?php echo $row_payment['payment_type_id']; ?>" <?php if(isset($_GET['id'])){if($row_sales_select['payment_type_id'] == $row_payment['payment_type_id']){echo "selected";}}?> ><?php echo $row_payment['payment_type'];?></option>
											<?php
												}
											?>
										</select>									
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input type="number" id="txt_shipping_amt" name="txt_shipping_amt" class="form-control" value = "<?php if(isset($_GET['id'])){ echo $row_sales_select['shipping_packing_amount']; }else {echo 0;}?><?php if(isset($_GET['quotation_to_sales'])){ echo $row_quotation['shipping_packing_amount']; }?>" onchange = "fnc_calculate_total()" placeholder="Enter Shipping/Packing Amount">																			
									</div>
								</div>
								<!--/span-->
							</div>	
							<div class="row">	
								<div class="col-md-4">
									<div class="form-group">
										...
										<textarea id="txt_narration" name="txt_narration" class="form-control" placeholder="Enter Narration"><?php if(isset($_GET['id'])){ echo $row_sales_select['narration']; }?><?php if(isset($_GET['quotation_to_sales'])){ echo $row_quotation['narration']; }?></textarea>
									</div>
								</div>
								<!--/span-->
								<div class="col-md-4">
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										<input type="checkbox" id="chk_is_round_off" name="chk_is_round_off" onchange = "fnc_calculate_total()" <?php if(isset($_GET['id'])){  if($row_sales_select['is_round_off'] == 1){ echo "checked";}}?>>	
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input type="number" id="txt_pay" name="txt_pay" class="form-control"  placeholder="Enter Pay Amount" value="<?php if(isset($_GET['id'])){ echo $row_sales_select['pay']; }?>" >	
									</div>
								</div>
								<!--/span-->
							</div>
						...
						<div class="form-actions">
							...
							<input type="hidden" name="sales_invoice_id" id="sales_invoice_id" value = "<?php if(isset($_GET['id'])){ echo $row_sales_select['sales_invoice_id']; }?>"/>
							<?php	if(isset($_GET['id']))
										{ ?>
...
$(document).ready(function(){
	...
	//remove button code
	  $(document).on('click', '.btn_remove', function(){  
		   ...
		   var detail_id = $(this).siblings("#sales_invoice_detail_id").val();
	...
	if(no_of_detail_row>0)
	{
		...
		success: function(data) 
		{
			for(i=0;i<no_of_detail_row;i++)
			{
				...
				document.getElementsByName("sales_invoice_detail_id[]")[i].value = data[i].sales_invoice_detail_id;
				...
				fnc_unit_change(document.getElementsByName("sales_invoice_detail_id[]")[i]);
	...
	if(no_of_detail_row_quotation>0)
	{
		$('#btn_reset').attr("disabled" , "disabled");
		$('#row0').remove();
		for(i=0;i<no_of_detail_row_quotation;i++)
			{	
				$('#dynamic_field').append('<tr id="unique"><?php if($setting->get_name('is_show_barcode') == 1) { ?><td><div class="form-group"><input type="number" readonly oninput="barcode_data(this)" name="barcode_id[]" id="barcode_id" class = "form-control" value = ""/></div>	</td><?php } ?><td><div class="form-group"><select class="form-control"  id="txt_product" name="txt_product[]" onchange="fnc_unit(this);fnc_style_remove(this);" placeholder = "select Product"><option value = "">--select--</option></option><?php $sql_autocomplete = "SELECT pm.*,ut.unit_name FROM tbl_product_master pm left join tbl_company tc on tc.company_id = pm.company_id left join tbl_unit ut on ut.unit_id= pm.unit_id where tc.is_default = 1";$rs_autocomplete = mysqli_query($con , $sql_autocomplete);while($row_autocomplete = mysqli_fetch_array($rs_autocomplete)){?><option value="<?php echo $row_autocomplete["product_id"];?>"><?php echo $row_autocomplete["product_name"];?>(<?php echo $row_autocomplete["unit_name"];?>)</option> <?php } ?></select></div></td><?php if($setting->get_name('is_show_serial') == 1) { ?><td><div class="form-group"><a id="link_serial" name="link_serial[]" onclick="fnc_serial_find(this);" class="form-control" data-toggle="modal" data-target="#serial">Serial</a></div></td><?php } ?><?php if($setting->get_name('is_show_batch') == 1) { ?><td><div class="form-group"><a id="link_batch"  name = "link_batch[]" class="form-control" onclick="fnc_batch_find(this);"  data-toggle="modal" data-target="#batch">Batch</a></div></td><?php } ?><td><div class="form-group"><select class="form-control" id="product_unit" name="product_unit[]" onchange="fnc_rate_update(this);"></select></div></td><td><div class="form-group"><input type="number" id="txt_quantity" name="txt_quantity[]" step="0.001" oninput = "fnc_qty_serial_count(this)" value = "" onchange = "tot();fnc_style_remove(this);" class="form-control" placeholder="Enter QUANTITY" ><input type="hidden" id="share_rate" name="share_rate[]"></div></td><td><div class="form-group"><input type="number" id="txt_rate" name="txt_rate[]" onchange = "tot();fnc_style_remove(this);" class="form-control" placeholder="Enter rate" ></div></td><td><div class="form-group"><input type="number" id="txt_discount[]" name="txt_discount[]" onchange = "tot();fnc_style_remove(this);" class="form-control" placeholder="Enter discount" ></div></td><td><div class="form-group"><select id="txt_gst" name="txt_gst[]" class="form-control" onchange="fnc_gst_cal(this);fnc_style_remove(this);"><option value = "">--select--</option><?php $sql_gst="SELECT gst.* FROM tbl_gstslab_master gst LEFT JOIN tbl_company com ON com.company_id  = gst.company_id  WHERE com.is_default = 1 "; $rs_gst = mysqli_query($con,$sql_gst); while($row_gst=mysqli_fetch_array($rs_gst)){ ?><option value="<?php echo $row_gst["gstslab_id"]; ?>"><?php echo $row_gst["gstslab_name"]; ?></option><?php } ?></select></div></td>	<td><div class="form-group"><input type="number" id="txt_total[]" name="txt_total[]" readonly = "true" class="form-control" placeholder="Enter TOTAL" ></div></td><td><input type="hidden" name="txt_discount_amt[]" id="txt_discount_amt"/><input type="hidden" name="txt_cgst[]" id="txt_cgst" value = "0"/><input type="hidden" name="txt_serial_no[]" id="txt_serial_no" value = "" > <input type="hidden" name="txt_batch_no[]" id="txt_batch_no" value = "" ><input type="hidden" name="sales_invoice_detail_id[]" id="sales_invoice_detail_id" value="" /> <button type="button" name="btn_remove" id="btn_remove" class="btn btn-danger btn_remove">X</button></td></tr>');
			}

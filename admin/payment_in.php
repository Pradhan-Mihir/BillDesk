...
$title = "BILL DESK-Payment In";
	...
	//receipt_no sql
	$sql_invoice = "select max(pi.receipt_no) 'receipt_no' from tbl_payment_in pi join tbl_financial_master fy on fy.financial_id = pi.financial_id where fy.is_default = 1";
	$rs_invoice = mysqli_query($con , $sql_invoice);
	$row_invoice = mysqli_fetch_array($rs_invoice);
	$receipt_no = $row_invoice['receipt_no'];
	if($receipt_no != '')
	...
	{		
		if($_POST['payment_in_id'] == '')
		{			
			if(!empty($_FILES["payment_in_img"]["name"]))
			{			
			$img3=$_FILES["payment_in_img"]["name"];
			$img3 = pathinfo($img3, PATHINFO_FILENAME).mt_rand(600000,999999).".".pathinfo($img3, PATHINFO_EXTENSION);				
			$tmp_name3=$_FILES["payment_in_img"]["tmp_name"];
			if(is_uploaded_file($tmp_name3))
			{
				copy($tmp_name3,"../images/payment_in_images/".$img3);
			}
			}
			else
			{
			...
			$sql_payment_in_iu = "insert into tbl_payment_in(company_id,financial_id,party_id,receipt_no,payment_type_id,cheque_ref_no,date,description,image,received,obj_name,obj_id) values('".$row_company_id['company_id']."','".$row_financial['financial_id']."','".$_POST['cmb_party']."','".$_POST['txt_receipt_no']."','".$_POST['cmb_payment_type']."','".$_POST['txt_cheque_ref']."','".$_POST['txt_date']."','".$_POST['txt_description']."','".$img3."','".$_POST['txt_received_amt']."','".$objname."','".$_POST['txt_receipt_no']."')";
			$rs_payment_in_iu = mysqli_query($con,$sql_payment_in_iu);
			...
			{
				echo "<script>window.location.href='invoice.php?payment_in_id=$last_id';</script>";
			}
		
			$sql_inv_id = "select max(payment_in_id) 'payment_in_id' from tbl_payment_in";
            ...
            if($_POST['txt_pur_inv_detail'] != '')
            {
                ...
                {
                    $varr = explode('-' , $arr[$i]);
//                    $varr[0] = purchase return primary
//                    $varr[1] = purchase return pay amt update
                    $sql_purchase_return_update = "update tbl_purchase_return_invoice set pay = pay +".$varr[1]." where purchase_return_invoice_id = '".$varr[0]."'";
                    mysqli_query($con , $sql_purchase_return_update);
					
					$sql_company_update = "update tbl_company_ledger set credit = credit +".$varr[1]." where related_id = '".$varr[0]."' and related_obj_name = 'purchase return' ";
					mysqli_query($con,$sql_company_update);
					
					$sql_party_update = "update tbl_party_ledger set credit = credit +".$varr[1]." where invoice_no = '".$varr[0]."' and invoice_type = 'purchase return' ";
            ...
            else if($_POST['txt_ser_inv_detail'] != '')
            {
                ...
                {
                    $varr = explode('-' , $arr[$i]);
//                  $varr[0] = purchase primary
//                  $varr[1] = purchase pay amt update
                    $sql_sales_update = "update tbl_sales_invoice set pay = pay +".$varr[1]." where sales_invoice_id = '".$varr[0]."'";
                    mysqli_query($con , $sql_sales_update);
					
					$sql_company_update = "update tbl_company_ledger set credit = credit +".$varr[1]." where related_id = '".$varr[0]."' and related_obj_name = 'sales' ";
					mysqli_query($con,$sql_company_update);
					
					$sql_party_update = "update tbl_party_ledger set credit = credit +".$varr[1]." where invoice_no = '".$varr[0]."' and invoice_type = 'sales' ";
					mysqli_query($con,$sql_party_update);
			...
			$sql_company_ledger="insert into tbl_company_ledger(company_id , related_id , related_obj_name,party_id ,date, details, credit, debit, financial_id, new_invoice_no) values('".$row_company_id['company_id']."' , '".$inv_id["payment_in_id"]."' ,'".$objname."','".$_POST['cmb_party']."' , '".$_POST['txt_date']."' , '".$_POST['txt_description']."' , '".$_POST['txt_received_amt']."','".$debit."','".$row_financial['financial_id']."','".$_POST['txt_new_invoice_no']."')";
			$rs_company_ledger = mysqli_query($con,$sql_company_ledger);
			
			//party_ledger
			$party_typ = "payment in";
			$inv_typ = "payment in"; 
			$credit = 0.00;
			
			$sql_party_ledger = "insert into tbl_party_ledger (company_id , party_type , party_id , invoice_type , invoice_no , detail , credit , debit , date , financial_id , new_invoice_no) VALUES('".$row_company_id['company_id']."' , '".$party_typ."' , '".$_POST['cmb_party']."' , '".$inv_typ."', '".$inv_id["payment_in_id"]."' , '".$_POST['txt_description']."' ,'".$credit."' , '".$_POST['txt_received_amt']."' , '".$_POST['txt_date']."' ,'".$row_financial['financial_id']."' ,'".$_POST['txt_new_invoice_no']."' ) ";
			...
			$sql_party_bal = "update tbl_party_master set opening_balance = '".$row_fetch_party_bal['opening_balance']."' - '".$_POST['txt_received_amt']."' where party_id = '".$_POST['cmb_party']."' ";
			$rs_party_bal = mysqli_query($con,$sql_party_bal);
			
		}
		else
		{	
			$sql_image_select = "select image from tbl_payment_in where payment_in_id = '".$_POST['payment_in_id']."' ";
			...
			$objname="payment in";
			//UPDATE CODE
			$sql_payment_in_update = "update tbl_payment_in set company_id ='".$row_company_id['company_id']."',financial_id='".$row_financial['financial_id']."',party_id='".$_POST['cmb_party']."',payment_type_id='".$_POST['cmb_payment_type']."',cheque_ref_no='".$_POST['txt_cheque_ref']."',date='".$_POST['txt_date']."',description='".$_POST['txt_description']."',image='".$img3."',received='".$_POST['txt_received_amt']."',obj_name='".$objname."',obj_id = '".$_POST['receipt_no']."' where payment_in_id ='".$_POST['payment_in_id']."' and obj_name='".$objname."' ";
			...
			$sql_company_ledger_id = "select *  from tbl_company_ledger where related_id = '".$_POST['payment_in_id']."' and related_obj_name = '".$objname."' ";
			$rs_company_ledger_id = mysqli_query($con,$sql_company_ledger_id);
			$row_company_ledger_id = mysqli_fetch_array($rs_company_ledger_id);
			
			$sql_company_ledger="update tbl_company_ledger set company_id = '".$row_company_id['company_id']."',related_id = '".$_POST['payment_in_id']."',related_obj_name='".$objname."',party_id = '".$_POST['cmb_party']."',date = '".$_POST['txt_date']."' ,details = '".$_POST['txt_description']."' , credit = '".$_POST['txt_received_amt']."' , debit = '".$debit."',financial_id = '".$row_financial['financial_id']."' where related_id = '".$_POST['payment_in_id']."' and related_obj_name = '".$objname."' ";
			$rs_company_ledger = mysqli_query($con,$sql_company_ledger);
			
			//party_ledger
			//select maximum party ledger id
			$sql_party_ledger_id = "select *  from tbl_party_ledger where invoice_no = '".$_POST['payment_in_id']."' ";
			$rs_party_ledger_id = mysqli_query($con,$sql_party_ledger_id);
			$row_party_ledger_id = mysqli_fetch_array($rs_party_ledger_id);
			
			$party_typ = "payment in";
			$inv_typ = "payment in"; 
			$credit = 0.00;
			
			$sql_party_ledger = "update tbl_party_ledger set company_id='".$row_company_id['company_id']."',party_type='".$party_typ."',party_id='".$_POST['cmb_party']."',invoice_type='".$inv_typ."',invoice_no='".$_POST['payment_in_id']."',detail='".$_POST['txt_description']."',credit='".$credit."',debit='".$_POST['txt_received_amt']."',date='".$_POST['txt_date']."',financial_id='".$row_financial['financial_id']."',new_invoice_no='".$_POST['txt_new_invoice_no']."' where invoice_no='".$_POST['payment_in_id']."' ";
			$rs_party_ledger = mysqli_query($con , $sql_party_ledger);
			...
			$sql_party_bal .= " '".$_POST['txt_received_amt']."' where party_id = '".$_POST['cmb_party']."' ";
			$rs_party_bal = mysqli_query($con,$sql_party_bal);
		...
		{
			echo "<script>window.location = 'payment_in_view.php';</script>";
		}	
	}
?>
<script type="text/javascript" language="javascript">
    ...
    function fnc_pr_sal_save_btn(e)
    {
        ...
        dat.value = '';
        document.getElementById('txt_received_amt').value = 0;
        for(let i=0;i<chk.length;i++)
        {
            if(chk[i].checked != 1)
                continue;
            ...
            document.getElementById('txt_received_amt').value = parseInt(document.getElementById('txt_received_amt').value) +  parseInt( txt[i].value );
        }
...
<div class="row">
	<div class="col-md-12">
		<div class="panel panel-info">
			<div class="panel-heading">Payment In</div>
			<div class="panel-wrapper collapse in" aria-expanded="true">
				<div class="panel-body">
					<form action="" method="post" name="frm_payment_in" id="frm_payment_in" enctype="multipart/form-data">
						<div class="form-body">								
							<div class="row">
								<div class="col-md-4">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input type="text" id="txt_receipt_no" name="txt_receipt_no" class="form-control" readonly value="<?php echo $receipt_no; ?>" >										
									</div>
								</div>
							</div>
							<!-- /row-->
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" id = "modal_close_btn" data-dismiss="modal"><span aria-hidden="true">&times;</span></button>
				<h4>Information Of Party</h4>
			</div>
			<div class="modal-body">
				<div class="col-lg-12 col-sm-12 col-xs-24">
					<ul class="nav customtab nav-tabs" role="tablist">
						<li role="presentation" class="nav-item"><a href="#purchase_content" id="purchase"  class="nav-link active" aria-controls="purchase" role="tab" data-toggle="tab" aria-expanded="true"><span class="visible-xs"><i class="ti-home"></i></span><span class="hidden-xs" > Purchase Return</span></a></li>
						<li role="presentation" class="nav-item"><a href="#sales_return_content" id="sales_return" class="nav-link" aria-controls="sales_return" role="tab" data-toggle="tab" aria-expanded="false"><span class="visible-xs"><i class="ti-settings"></i></span> <span class="hidden-xs">Sales</span></a></li>
					</ul>
					<!-- Tab panes -->
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane fade" id="purchase_content" >
							<div class="col-md-12">
...
<!-- /model End---->
							...
							<div class="row">
								<div class="col-md-4">
									<div class="form-group">
										...
										<input type="file" id="input-file-now-custom-1" class="dropify"  name="payment_in_img" />							
									</div>
								</div>
								<!-- /span---->
								<div class="col-md-4">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input type="number" id="txt_received_amt" name="txt_received_amt" readonly class="form-control" placeholder="Enter Received Amount" >										
									</div>
								</div>
							</div>
							<!-- /row-->
						...
						<div class="form-actions">
							<div class = "row">
								...
								<input type="hidden" name="payment_in_id" id="payment_in_id" value="<?php if(isset($_GET['id'])){ echo base64_decode($_GET['id']);} ?>"/> 
...
<script>
    ...
    $('#payment_in_img').on('change', function() {
      $('.dropify-clear').css("display", "block");
      $('.dropify-loader').css("display", "none");
      $('.dropify-preview').css("display", "block");
    });
	...
	var payment_in_id = $('#payment_in_id').val();
	
	if(payment_in_id != 0 )
	{
		var imagePath1 = "../images/payment_in_images/";
		$.ajax({
			url:"payment_in_ajax.php",
			method:"POST",  
			data:{ 'id' : payment_in_id , 'edit' : 1 },
			success:function(data)  
			{  
				//alert("inside_success");
				const obj = JSON.parse(data);
				console.log(obj);
				document.getElementById("payment_in_id").value = payment_in_id;
				...
				$('#payment_in_img').attr("data-default-file",imagePath1 + imagePath2);
				$('.dropify-preview').css("display","block");
				...
				$('#payment_in_img').attr('disabled', 'disabled');
				document.getElementById("txt_received_amt").value = obj.received;

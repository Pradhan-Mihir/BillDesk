<?php
$title = "BILL DESK-Sales Return Invoice";
...
foreach ($row_product_setting as $key => $value)
{
	...
	if(isset($_GET['id']))
	{
		...
		$sales_return_id=base64_decode($_GET['id']);
		$sql_sales_return_select="SELECT * FROM tbl_sales_return WHERE sales_return_id ='".$sales_return_id."'";
		$rs_sales_return_select=mysqli_query($con,$sql_sales_return_select);
		$row_sales_return_select=mysqli_fetch_array($rs_sales_return_select);
		
		//fetch data for invoice detail
		$sql_sales_return_detail_select = "select * from tbl_sales_return_detail where sales_return_id = '".$sales_return_id."'";
		$rs_sales_return_detail_select = mysqli_query($con , $sql_sales_return_detail_select);
		$detail_row_num = mysqli_num_rows($rs_sales_return_detail_select);
	...
	//invoice_no sql
	$sql_invoice = "select max(pr.invoice_no) 'invoice_no' from tbl_sales_return pr join tbl_financial_master fy on fy.financial_id = pr.financial_id where fy.is_default = 1";
	...
	{
		global $row_sales_return_select;
		
		if(isset($_GET['id']))
		{
			if($row_sales_return_select['state_of_supply'] == $name)
			{echo "selected";}
		}
	}
		  ...
		  for($i=0; $i<$number; $i++)  
			{  
			   if(trim($_POST["txt_rate"][$i] != ''))
				{  
					...
					$sql_stock_sales = "update tbl_product_master set closing_stock = (select closing_stock from tbl_product_master where product_id ='". $_POST["txt_product"][$i]."' ) - '".$qty_for_stock."' where product_id = '". $_POST["txt_product"][$i]."' ";
			...
			else
			{
				if(isset($_POST['btn_save_n_print']))
					echo "<script>window.location.href='invoice.php?sales_return_id=$inv_id';</script>";
				else
					echo "<script>window.location.href='sales_return.php';</script>";
			}
		}
	}
//``````````````````````````````````````````````````````UPDATE```````````````````````````````````````````````````````````````````
	if(isset($_POST['btn_edit']) || isset($_POST['btn_update_n_print']))
	{
        ...
        $objname = "sales_return";
		$img3 = '';
		$sql_sales_invoice_iu = "UPDATE tbl_sales_return SET party_id = '".$_POST['cmb_party']."',state_of_supply ='".$_POST['cmb_state_of_supply']."' , sales_return_date = '".$_POST['txt_invoice_date']."' , payment_type_id = '".$_POST['cmb_payment_type']."', narration= '".$_POST['txt_narration']."', sub_total = '".$_POST['txt_sub_total']."', shipping_packing_amount= '".$_POST['txt_shipping_amt']."' , is_round_off = '".$chk_round_off."', round_off = '".$_POST['txt_round_off']."' , total = '".$_POST['txt_total_inv']."', pay = '".$_POST['txt_pay']."' , new_invoice_no ='".$new_inv_no."' ,is_gst_bill = '".$row_product_setting['is_gst_bill']."'  WHERE sales_return_id = '".$_POST['sales_return_id']."'";
		$rs_sales_return_update = mysqli_query($con , $sql_sales_invoice_iu);
		//echo $sql_sales_invoice_iu;
		
		$sql_payment_out_update = "update tbl_payment_in set company_id ='".$row_company_id['company_id']."',financial_id='".$row_financial_id['financial_id']."',party_id='".$_POST['cmb_party']."',payment_type_id='".$_POST['cmb_payment_type']."',cheque_ref_no='".$cheque."',date='".$_POST['txt_invoice_date']."',description='".$_POST['txt_narration']."',image='".$img3."',paid ='".$_POST['txt_pay']."',obj_name='".$objname."',obj_id = '".$_POST['sales_return_id']."' where party_id ='".$_POST['cmb_party']."' and obj_name = '".$objname."' and obj_id = '".$_POST['sales_return_id']."'";
		$rs_payment_out_update = mysqli_query($con,$sql_payment_out_update);
		echo $sql_payment_out_update;
		//company_ledger
	  
		$credit=0.00;
		  
		$sql_company_ledger="update tbl_company_ledger set company_id = '".$row_company_id['company_id']."',related_id = '".$_POST['sales_return_id']."',related_obj_name='".$objname."',party_id = '".$_POST['cmb_party']."',date = '".$_POST['txt_invoice_date']."' ,details = '".$_POST['txt_narration']."' , credit = '".$credit."' , debit = '".$_POST['txt_pay']."' ,financial_id = '".$row_financial_id['financial_id']."' where related_id = '".$_POST['sales_return_id']."' and related_obj_name = '".$objname."' ";
		$rs_company_ledger = mysqli_query($con,$sql_company_ledger);
		...
		{  
		  for($i=0; $i<$number; $i++)  
		  {
		   ...
		   if($_POST["sales_return_detail_id"][$i] == '')		   
			{
				//serial_no is available or not. checking
				$sr = '';
				if($_POST['txt_serial_no'][$i] != '')
			...
			$inv_id = $_POST['sales_return_id'];
		 	if(!$rs_sales_return_update)
			{
			...
			else
			{
				if(isset($_POST['btn_update_n_print']))
					echo "<script>window.location.href='invoice.php?sales_return_id=$sales_return_id';</script>";
				else
...
var swit = 0;
	...
	function tot()
	{
		...
		{	if(prd_rate[i].value == 0 && prd_qty[i].value == 0  && prd_rate[i].value == '' && prd_qty[i].value == '' )
			...
			if(prd_disc[i].value != '' && prd_disc[i].value != 0)
			{
				...
				disc_amt = parseFloat(total_val) * parseFloat(disc_ratio);
				document.getElementsByName("txt_discount_amt[]")[i].value = disc_amt.toFixed(2);
				total_val = parseFloat(total_val) - parseFloat(disc_amt);
			}
			//console.log(total_val);
			...
			{
				total_val= parseFloat(total_val) + (parseFloat(document.getElementsByName('txt_cgst[]')[i].value / 100) * parseFloat(total_val) );
			}
			sub_total += total_val;
			//console.log(total_val);
	...
	function fnc_unit(e)
	{
		...
		let prd_detail_id = document.getElementsByName("sales_return_detail_id[]");
...
function barcode_data(e)
	{
		...
		if(barcode_no == '' || barcode_no.length < 13 || barcode_no.length > 13)
		{
		...
		$.ajax({  
			url:"sales_return_ajax.php",
			type:"POST",  
			data:{ 'id' : barcode_no ,'barcode' :1  , 'party' : party},
			datatype:'json',
			async:false, 
			success:function(data)
				{
					...
					fnc_unit(prd[index]);
					fnc_unit_change(prd[index]);

					$('#btn_add').click();
					document.getElementsByName('barcode_id[]')[prd.length - 1].focus();
				...
				<div class="panel-body">
					<form action="" method="post" name="add_name" id = "add_name" enctype="multipart/form-data">
							...
							<div class="row">
								<div class="col-md-3">
									<div class="form-group" id = "switch_party">
										...
										<select id="cmb_party" name="cmb_party"  class="form-control" onchange="fnc_party();">
											...
											<?php 
												$party="select pm.party_id , pm.party_name from tbl_sales_invoice pi left join tbl_party_master pm on pm.party_id = pi.party_id where pm.party_type = 1 group by pi.party_id";
												$rs_party=mysqli_query($con,$party);
												while($row_party=mysqli_fetch_array($rs_party))
												{ 
											?>
												<option value="<?php echo $row_party['party_id']; ?>" <?php if(isset($_GET['id'])){if($row_sales_return_select['party_id'] == $row_party['party_id']){echo "selected";}}?> ><?php echo $row_party['party_name'];?></option>
											<?php
												} 
											?>
										</select>								
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="text" id="txt_invoice_no" name="txt_invoice_no" class="form-control" placeholder="Enter Invoice No." readonly = "true"value="<?php if(isset($_GET['id'])){ echo $row_sales_return_select['invoice_no']; }else { echo $invoice_no; } ?>">										
									</div>
								</div>
								<!--/span-->
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="hidden" name="is_out_off_state" id="is_out_off_state" value="<?php if(isset($_GET['id'])){ echo $row_sales_return_select['out_of_state']; }?>"/>
									</div>
								</div>
								<!--/span-->
								<div class="col-md-3">
							...
							<div class = "row">
								...
								<?php	if(!isset($_GET['id']))
									...
									<tr id='row0'>
										...
										<td>
											<div class="form-group">
												<input type="number" id="txt_quantity" name="txt_quantity[]" step="0.001" value = "0" oninput = "fnc_qty_serial_count(this)" onchange = "tot();fnc_style_remove(this); fnc_qty_maintain();" onkeyup="fnc_qty_maintain();"  class="form-control" placeholder="Enter QUANTITY" >										
										...
										<td>
											<div class="form-group">
												<input type="number" id="txt_discount" name="txt_discount[]" step="0.01" onchange = "tot();fnc_style_remove(this);"  class="form-control" placeholder="Enter discount" >										
											</div>
										</td>
										<td>
											<div class="form-group">
										...
										<td>
											<div class="form-group">
												<input type="number" id="txt_total" name="txt_total[]" step="0.01" readonly = "true" class="form-control" value = "0" placeholder="Enter TOTAL" >										
											</div>
										</td>
										<td>
										...
										<input type="hidden" name="sales_return_detail_id[]" id="sales_return_detail_id" value = ''/> 
						...
						<div class="row">
								...
								<div class="form-group">
									...
									<select id="cmb_payment_type" name="cmb_payment_type" class="form-control" onchange="fnc_payment_type_text();">
										<?php 
											...
											{ 
										?>
											<option value="<?php echo $row_payment['payment_type_id']; ?>" <?php if(isset($_GET['id'])){if($row_sales_return_select['payment_type_id'] == $row_payment['payment_type_id']){echo "selected";}}?> ><?php echo $row_payment['payment_type'];?></option>
										<?php
											}
										?>
									</select>										
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<input type="number" id="txt_shipping_amt" name="txt_shipping_amt" class="form-control" value = "<?php if(isset($_GET['id'])){ echo $row_sales_return_select['shipping_packing_amount']; }else {echo "0.00";}?>" onchange = "fnc_calculate_total()" placeholder="Enter Shipping/Packing Amount">																			
								</div>
							</div>
							<!--/span-->
						</div>	
							<div class="row">	
								...
								<div class="form-group">
									...
									<textarea id="txt_narration" name="txt_narration" class="form-control" placeholder="Enter Narration"><?php if(isset($_GET['id'])){ echo $row_sales_return_select['narration']; }?></textarea>
								</div>
							</div>
								<!--/span-->
								<div class="col-md-4">
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										<input type="checkbox" id="chk_is_round_off" name="chk_is_round_off" onchange = "fnc_calculate_total()" <?php if(isset($_GET['id'])){  if($row_sales_return_select['is_round_off'] == 1){ echo "checked";}}?>>	
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<input type="number" id="txt_pay" name="txt_pay" class="form-control"  placeholder="Enter Pay Amount" value="<?php if(isset($_GET['id'])){ echo $row_sales_return_select['pay']; }?>" >	
									</div>
								</div>
								<!--/span-->
							</div>
						...
						<div class = "row">
							...
							<input type="hidden" name="sales_return_id" id="sales_return_id" value = "<?php if(isset($_GET['id'])){ echo $row_sales_return_select['sales_return_id']; }?>"/>
							<?php	if(isset($_GET['id']))
										{ ?>
...
$(document).ready(function(){
	...
	//remove button code
	  $(document).on('click', '.btn_remove', function(){  
	    let detail_id = $(this).siblings("#sales_return_detail_id").val();
	  if(detail_id != '')
	  {
	...
	success: function(data) 
	{
		for(i=0;i<no_of_detail_row;i++)
		{
			 var detail_id = $(this).siblings("#sales_return_detail_id").val();
			//console.log(data[i]);
			...
			document.getElementsByName("sales_return_detail_id[]")[i].value = data[i].sales_return_detail_id;
			...
			fnc_unit_refresh(document.getElementsByName("sales_return_detail_id[]")[i]);

...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertBarcode_master` (IN `CID` BIGINT(20), IN `BCODE` VARCHAR(100), IN `PNAME` VARCHAR(100), IN `PCODE` VARCHAR(100), IN `GID` BIGINT(20), IN `SRATE` DECIMAL(18,2), IN `MDATE` DATE, IN `EDATE` DATE, IN `SBARCODE` INT(11), IN `PBARCODE` INT(11))  BEGIN 

INSERT INTO tbl_barcode_master (company_id , barcode , product_name ,product_code ,gstslab_id,sales_rate,mfg_date,exp_date,start_barcode_from,print_barcode_at) VALUES(CID , BCODE , PNAME , PCODE , GID ,SRATE , MDATE , EDATE , SBARCODE , PBARCODE);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertExpence` (IN `NAME` VARCHAR(100), IN `CATEGORY` BIGINT(20), IN `AMT` BIGINT(20), IN `FID` BIGINT(20))  BEGIN
	INSERT INTO tbl_expence (expence_name , expense_id , expence_amount,financial_id ) VALUES (NAME , CATEGORY , AMT,FID);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `viewBarcode_master` ()  BEGIN
	SELECT * FROM tbl_barcode_master;
END$$
...
SELECT se.expense_name , fin.financial_name , ce.* 
FROM tbl_expence ce 
...
CREATE TABLE `tbl_barcode_master` (
  ...
  `is_show_barcode` tinyint(4) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
(2, 4, '7410852963', 'CPC', 'C101', 1, '50000.00', '2021-12-01', '2030-12-01', 1, 20, 1),
(3, 0, 'sdasdfgh', '', '', 0, '0.00', '0000-00-00', '0000-00-00', 0, 0, 0);
...
(3, 'mihir', 9376521396, 4185123685, 'mihir.pradhan.786@gmail.com', 'Pandesara', 'SURAT', 'Gujarat', 394221, 'fghj', 'dfghj', 84518451, '7yfugi', 'pann', 'ybunk9', 'csttt', '8451', 'gybjn845', 'IMG_4455650037927927.PNG', 0),
(4, 'bhautik', 9637521485, 6541239650, 'mihir.pradhan.786@gmail.com', 'Pandesara', 'SURAT', 'Gujarat', 394221, 'fghj', 'dfghj', 52146325, '7yfugi', 'bca', 'gyujnk', 'abc', 'yuhbj', 'gybjn845', 'photo_2021-12-07_16-12-40686798828835.jpg', 1);
...
INSERT INTO `tbl_expence` (`expence_id`, `expence_name`, `expense_id`, `expence_amount`, `expence_date`, `financial_id`) VALUES
(3, 'xerox', 3, 56, '2021-12-17', 8),
(4, 'chai', 2, 50, '2021-12-17', 8),
(5, 'water line', 1, 51659, '2021-12-17', 19),
(6, 'be fees', 2, 8520, '2021-12-17', 20);
...
--
ALTER TABLE `tbl_barcode_master`
  MODIFY `barcode_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
...
--
ALTER TABLE `tbl_expence`
  MODIFY `expence_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
ALTER TABLE `tbl_unit`
  MODIFY `unit_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
COMMIT;
